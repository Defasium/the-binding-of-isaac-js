...
const getInputs = () =>{
	let input = {hp: Player.hp, playerX: Player.x, playerY: Player.y,
				   bossDx: 1000, bossDy: 1000, bossD: -1000, bossAngle: 0,
				   minNearDx: 1000, minNearDy: 1000, minNearD: -1000, minNearAngle: 0,
				   enNearDx: 1000, enNearDy: 1000, enNearD: -1000, enNearAngle: 0};
	const sortMap = {false: 1, true: -1};
...
const getAction = async (inputs) => {
	...
	const funcMap = {false: indexOfMax, true: indexOfProb};
	//console.log(actions);
...
setInterval(async () => {
	...
	//console.log(actions);
	const keyMap = {false: 'keyup', true: 'keydown'};
...
const saveRecords = (records) => {
	...
	const blobURI = window.URL.createObjectURL(new Blob ([JSON.stringify(records.map(e=>e.map(v=>Number(v.toPrecision(3)))))], {type:'application/json'}));
